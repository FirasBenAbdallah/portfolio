// Translate Aside
export const aside = {
  // Location_localisation
  location: {
    english: "Ariana, Tunisia",
    french: "Ariana, Tunisie",
  },
  // Education_éducation
  title: {
    english: "Education",
    french: "Éducation",
  },
};

// Translate Skills
export const skills = {
  title: {
    english: "Skills",
    french: "Compétences",
  },
  theme1: {
    english: "• Programming languages :",
    french: "• Langages de programmation :",
  },
  theme2: {
    english: "• Web development :",
    french: "• Développement web :",
  },
  theme3: {
    english: "• Mobile development:",
    french: "• Développement mobile :",
  },
  theme4: {
    english: "• Database design and management:",
    french: "• Conception et gestion de bases de données :",
  },
  theme5: {
    english: "• DevOps:",
    french: "• DevOps :",
  },
  theme6: {
    english: "• Unit Testing:",
    french: "• Test Unitaire :",
  },
  theme7: {
    english: "• AI:",
    french: "• IA :",
  },
  link: {
    english: "Learn more on Github",
    french: "En savoir plus sur Github",
  },
};

// Translate Testimonial
export const testimonial = {
  title: {
    english: "Testimonials",
    french: "Témoignages",
  },
  description: {
    english:
      "Firas Ben Abdallah stands out for his deep expertise as a computer engineer. His professional rigor and dedication to the craft ensure the success of every project he undertakes. Entrusting him with a task ensures its flawless execution",
    french:
      "Firas Ben Abdallah se distingue par son expertise approfondie en tant qu'ingénieur informatique. Sa rigueur professionnelle et son dévouement au métier garantissent la réussite de chaque projet qu'il entreprend. Le confier une mission, c'est assurer sa réalisation à la perfection.",
  },
  link: {
    english: "Learn more on LinkedIn",
    french: "En savoir plus sur LinkedIn",
  },
};

// Translate Education
export const education = [
  {
    title: {
      english: "Software Engineering",
      french: "Ingénieur en informatiques",
    },
    name: {
      english: "ESPRIT",
      french: "ESPRIT",
    },
    date: "| 2021-2024",
  },
  {
    title: {
      english: "Engineer in Applied Sciences for Weapons",
      french: "Ingénieur en sciences appliquées aux armes",
    },
    name: {
      english: "Military Academies of Fondek Jdid",
      french: "Académies militaires Fondek Jdid",
    },
    date: "| 2012-2016",
  },
  {
    title: {
      english: "Preparatory School for Physics and Chemistry",
      french: "Ecole Préparatoire Physique-Chimie",
    },
    name: {
      english: "EPAM, Sousse",
      french: "EPAM, Sousse",
    },
    date: "| 2012-2016",
  },
  {
    title: {
      english: "Mathematics Baccalaureate",
      french: "Baccalauréat en Mathématiques",
    },
    name: {
      english: "Slimane Ben Slimane High School, Zaghouan",
      french: "Lycée Slimane Ben Slimane, Zaghouan",
    },
    date: "| 2009-2010",
  },
];

// Translate Languages
export const languages = {
  title: {
    english: "Languages",
    french: "Langues",
  },
  language1: {
    english: "Arabic",
    french: "Arabe",
  },
  language2: {
    english: "French",
    french: "Français",
  },
  language3: {
    english: "English",
    french: "Anglais",
  },
  type1: {
    english: "Native speaker",
    french: "Locuteur natif",
  },
  type2: {
    english: "Professional skills",
    french: "Competances professionnelles",
  },
};
